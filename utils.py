...
from test_config import Configuration, SearchAPI
from test_prompts import summarize_webpage_prompt
from test_state import ResearchComplete, Summary
...
async def tavily_search_async(
    ...
    if not api_key:
        return [{"url": "error", "content": "Error: TAVILY_API_KEY is missing. Please check your .env file."}]
...
def gemini_websearch_called(message: MessageLikeRepresentation) -> bool:
    """Detect if Google Gemini's native web search (grounding) was used in the response.
    
    Args:
        message: The response message object from the model
        
    Returns:
        True if web search/grounding was used, False otherwise
    """
    try:
        ...
        # Check for Google/Gemini grounding metadata
        # Pattern 1: Direct groundingMetadata in usage_metadata or at root
        if "groundingMetadata" in metadata:
             return True
             
        # Pattern 2: Nested under safety_ratings or other Google specific structures
        # (This varies by LangChain version and specific Gemini API version)
        
        # Pattern 3: Explicit search entry in tool usage if standard tool pattern used
        # (Though native grounding often appears differently)
...
MODEL_TOKEN_LIMITS = {
    "google:gemini-3-pro": 2097152,
    "google:gemini-3-flash": 1048576,
    "google:gemini-2.5-pro": 262144,
...
def _get_api_key_from_source(key_name: str, config: RunnableConfig) -> Optional[str]:
    """Helper to fetch API key from environment or config (robust fallback)."""
    # 1. Try environment variable first
    env_val = os.getenv(key_name)
    if env_val:
        return env_val
        
    # 2. Try fetching from config if available
    if config:
        configurable = config.get("configurable", {})
        
        # Check inside 'apiKeys' dict (if structured that way)
        api_keys = configurable.get("apiKeys", {})
        if isinstance(api_keys, dict) and key_name in api_keys:
             return api_keys[key_name]
             
        # Check at the top level of configurable (common in simple setups)
        if key_name in configurable:
            return configurable[key_name]
...
def get_api_key_for_model(model_name: str, config: RunnableConfig):
    ...
    if model_name.startswith("openai:"):
        return _get_api_key_from_source("OPENAI_API_KEY", config)
    elif model_name.startswith("anthropic:"):
        return _get_api_key_from_source("ANTHROPIC_API_KEY", config)
    elif model_name.startswith("google"):
        return _get_api_key_from_source("GOOGLE_API_KEY", config)
    return None

def get_tavily_api_key(config: RunnableConfig):
    """Get Tavily API key from environment or config."""
    return _get_api_key_from_source("TAVILY_API_KEY", config)
